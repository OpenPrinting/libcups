     ...
     char *argv[])			// I - Command-line arguments
{
      ...
      for (opt = argv[i] + 1; *opt; opt ++)
      {
        switch (*opt)
        {
          case 'C' : // -C COUNTRY
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing country after '-C'."));
                return (usage(stderr));
	      }
	      country = argv[i];
	      break;
          ...
          case 'L' : // -L LOCALITY
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing locality/city/town after '-L'."));
                return (usage(stderr));
	      }
	      locality = argv[i];
	      break;
          ...
          case 'O' : // -O ORGANIZATION
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing organization after '-O'."));
                return (usage(stderr));
	      }
	      organization = argv[i];
	      break;
          ...
          case 'R' : // -R CSR-FILENAME
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing CSR filename after '-R'."));
                return (usage(stderr));
	      }
	      csrfile = argv[i];
	      break;
          ...
          case 'S' : // -S STATE
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing state/province after '-S'."));
                return (usage(stderr));
	      }
	      state = argv[i];
	      break;
          ...
          case 'U' : // -U ORGANIZATIONAL-UNIT
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing organizational unit after '-U'."));
                return (usage(stderr));
	      }
	      org_unit = argv[i];
	      break;
          ...
          case 'a' : // -a SUBJECT-ALT-NAME
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing subjectAltName after '-a'."));
                return (usage(stderr));
	      }
	      if (num_alt_names >= (sizeof(alt_names) / sizeof(alt_names[0])))
	      {
	        cupsLangPuts(stderr, _("cups-x509: Too many subjectAltName values."));
	        return (1);
	      }
          ...
          case 'd' : // -d DAYS
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing expiration days after '-d'."));
                return (usage(stderr));
	      }
	      if ((days = atoi(argv[i])) <= 0)
	      {
	        cupsLangPrintf(stderr, _("cups-x509: Bad DAYS value '%s' after '-d'."), argv[i]);
	        return (1);
	      }
	      break;
          ...
          case 'p' : // -p PURPOSE
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing purpose after '-p'."));
                return (usage(stderr));
	      }
	      purpose = 0;
	      ...
	      if (strstr(argv[i], "OCSPSigning"))
	        purpose |= CUPS_CREDPURPOSE_OCSP_SIGNING;
              if (purpose == 0)
              {
                cupsLangPrintf(stderr, _("cups-x509: Bad purpose '%s'."), argv[i]);
                return (usage(stderr));
	      }
	      break;
          ...
          case 'r' : // -r ROOT-NAME
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing root name after '-r'."));
                return (usage(stderr));
	      }
	      root_name = argv[i];
	      break;
          ...
          case 't' : // -t TYPE
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing certificate type after '-t'."));
                return (usage(stderr));
	      }
	      ...
	      {
	        cupsLangPrintf(stderr, _("cups-x509: Bad certificate type '%s'."), argv[i]);
	        return (usage(stderr));
	      }
	      break;
          ...
          case 'u' : // -u USAGE
              ...
              if (i >= argc)
              {
                cupsLangPuts(stderr, _("cups-x509: Missing key usage after '-u'."));
                return (usage(stderr));
	      }
	      keyusage = 0;
	      ...
	      if (keyusage == 0)
	      {
	        cupsLangPrintf(stderr, _("cups-x509: Bad key usage '%s'."), argv[i]);
	        return (usage(stderr));
	      }
	      break;

          default :
              cupsLangPrintf(stderr, _("%s: Unknown option '-%c'."), "cups-x509", *opt);
              return (usage(stderr));
	}
      }
    }
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unknown command '%s'."), command);
    return (usage(stderr));
  }
}
      ...
      int        days)			// I - Number of days
{
  ...
  if (csrfile)
  {
    ...
    if (csrfd < 0)
    {
      cupsLangPrintf(stderr, _("cups-x509: Unable to access '%s': %s"), csrfile, strerror(errno));
      return (1);
    }

    if (fstat(csrfd, &csrinfo))
    {
      cupsLangPrintf(stderr, _("cups-x509: Unable to stat '%s': %s"), csrfile, strerror(errno));
      close(csrfd);
      return (1);
    }
    ...
    {
      cupsLangPrintf(stderr, _("cups-x509: Unable to allocate memory for '%s': %s"), csrfile, strerror(errno));
      close(csrfd);
      return (1);
    }
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: No request for '%s'."), common_name);
    return (1);
  }
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to create certificate (%s)"), cupsGetErrorString());
    free(request);
    return (1);
  }
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to get generated certificate for '%s'."), common_name);
    return (1);
  }
    ...
    int                days)		// I - Number of days until expiration
{
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to get generated private key for '%s'."), common_name);
    return (1);
  }
    ...
    const char         **alt_names)	// I - subjectAltName's
{
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to create certificate request (%s)"), cupsGetErrorString());
    return (1);
  }
  ...
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to get generated certificate request for '%s'."), common_name);
    return (1);
  }
...
do_server(const char *host_port)	// I - Hostname/port
{
  ...
  if (num_listeners == 0)
  {
    cupsLangPrintf(stderr, _("cups-x509: Unable to listen on port %d: %s"), port, cupsGetErrorString());
    return (1);
  }
  ...
  for (;;)
  {
    ...
    for (i = 0, http = NULL; i < num_listeners; i ++)
    {
      if (listeners[i].revents & POLLIN)
      {
        ...
        cupsLangPrintf(stderr, _("cups-x509: Unable to accept connection: %s"), cupsGetErrorString());
      }
    }
    ...
    {
      cupsLangPrintf(stderr, _("cups-x509: Unable to encrypt connection: %s"), cupsGetErrorString());
      httpClose(http);
      continue;
    }
	...
	cupsLangPuts(stderr, _("cups-x509: Client closed connection."));
      else
	cupsLangPrintf(stderr, _("cups-x509: Bad request line (%s)."), strerror(httpGetError(http)));
    }
    else if (state == HTTP_STATE_UNKNOWN_METHOD)
    {
      cupsLangPuts(stderr, _("cups-x509: Bad/unknown operation."));
    }
    else if (state == HTTP_STATE_UNKNOWN_VERSION)
    {
      cupsLangPuts(stderr, _("cups-x509: Bad HTTP version."));
    }
    else
    {
...
usage(FILE *out)			// I - Output file (stdout or stderr)
{
  ...
  cupsLangPuts(out, _("--pin                          Pin certificate found by client command"));
  cupsLangPuts(out, _("--require-ca                   Require CA-signed certificate for client command"));
  cupsLangPuts(out, _("--version                      Show the program version"));
  cupsLangPuts(out, _("-C COUNTRY                     Set country."));
  cupsLangPuts(out, _("-L LOCALITY                    Set locality name."));
  cupsLangPuts(out, _("-O ORGANIZATION                Set organization name."));
  cupsLangPuts(out, _("-R CSR-FILENAME                Specify certificate signing request file."));
  cupsLangPuts(out, _("-S STATE                       Set state."));
  cupsLangPuts(out, _("-U ORGANIZATIONAL-UNIT         Set organizational unit name."));
  cupsLangPuts(out, _("-a SUBJECT-ALT-NAME            Add a subjectAltName."));
  cupsLangPuts(out, _("-d DAYS                        Set expiration date in days."));
  cupsLangPuts(out, _("-p PURPOSE                     Comma-delimited certificate purpose\n"
                      "                               (serverAuth, clientAuth, codeSigning, emailProtection, timeStamping, OCSPSigning)"));
  cupsLangPuts(out, _("-r ROOT-NAME                   Name of root certificate"));
  cupsLangPuts(out, _("-t TYPE                        Certificate type\n"
                      "                               (rsa-2048, rsa-3072, rsa-4096, ecdsa-p256, ecdsa-p384, ecdsa-p521)"));
  cupsLangPuts(out, _("-u USAGE                       Comma-delimited key usage\n"
