...
// implementations expose the (more logical) 32-bit address parts to everyone,
// while others only expose it to kernel code...  To make supporting IPv6 even
// easier, each vendor chose different core structure and union names, so the
// same defines or code can't be used on all platforms.
//
// The following will likely need tweaking on new platforms that support IPv6 -
// the "s6_addr32" define maps to the 32-bit integer array in the in6_addr
// union, which is named differently on various platforms.
//
...
extern char		*httpAddrLookup(const http_addr_t *addr, char *name, size_t namelen) _CUPS_PUBLIC;
...
extern const char	*httpGetHostname(http_t *http, char *s, size_t slen) _CUPS_PUBLIC;
...
extern bool		httpWriteResponse(http_t *http, http_status_t status) _CUPS_PUBLIC;
